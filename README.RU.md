...
> 📋 **Подробная документация**: [bermudaphp/finder](https://github.com/bermudaphp/classFinder) | [Руководство на русском](https://github.com/bermudaphp/classFinder/blob/master/README.RU.md)
...
RouteRecord предоставляет удобный доступ к различным компонентам маршрута:
...
)); // обаботчик закешируется, но возникнет ошибка, т.к. отсутствует контекст // Undefined variable $service
...
2. **Передавайте контекстные данные,** если испольюзается use в обработчиках-замыканиях

Наиболее предпочтительный вариант обработчика - имя класса ```MyHandler::class``` или сочетание имени и метода ```MyHandler::handle```
